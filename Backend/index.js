...
mongoose
  .connect("mongodb+srv://Lomukisa:test@cluster0.sxgczzt.mongodb.net/EmployeeDB?retryWrites=true&w=majority")
  .then(() => {
    console.log("Connected to MongoDBAtlas 2024");
  })
  .catch((error) => {
    console.log("Error connecting to MongoDBAtlas", error);
  });
...
app.post("/addEmployee", async (req, res) => {
  try {
    ...
    //saving new employee into mongodbAtlas
...
app.get("/employees", async (req, res) => {
  try {
    ...
    res.status(200).json({message: "Retieved all employess", employee: employees});
  } catch (error) {
...
app.post("/attendance", async (req, res) => {
  try {
    ...
    //finding an existing employee with id and date..
    ...
    if (existingAttendance) {
      ...
      res.status(200).json({message: "New attendance added succesfully!!", newattendance:newAttendance});
    }
  } catch (error) {
    res.status(500).json({ message: "Error submitting new attendance" });
  }
});
...
app.get("/attendance", async (req, res) => {
  try {
    ...
    res.status(200).json({message: "attendance data got successfully!!",attendanceData: attendanceData});
  } catch (error) {
...
app.get("/attendance-report-all-employees", async (req, res) => {
    try {
      ...
      res.status(200).json({message: "Generated a report successfully", report: report });
    } catch (error) {
